...
using System.Xml;
using Lesse.LoadRunner.SequenceModel;
using Rule = Lesse.LoadRunner.SequenceModel.Rule;
...
namespace Lesse.LoadRunner.ParameterEditor
{
    ...
    public partial class ParameterEditorWindow : Window
    {
            ...
            foreach (XmlNode node in doc.SelectNodes("CorrelationSettings", ns))
            {
                foreach (XmlNode groupNode in node.SelectNodes("Group", ns))
                {
                    foreach (XmlNode rul in groupNode.SelectNodes("Rule", ns))
                    {
                        ...
                        Rule r = new Rule(rul.Attributes["Name"].InnerText.ToLower().Trim());
        ...
        private bool ContainsRule(Rule r)
        {
            foreach (Rule item in saveParameters.Rules)
            {
                if (r.Name == item.Name)
                {
                    return true;
        ...
        {
            foreach (Rule r in this.saveParameters.Rules)
            {
        ...
        private void loadCorrelationInformation(XmlDocument doc, XmlNamespaceManager ns)
        {
            ...
            foreach (XmlNode node in doc.SelectNodes("CorrelationSettings", ns))
            {
                ...
                foreach (XmlNode groupNode in node.SelectNodes("Group", ns))
                {
                    ...
                    foreach (Rule r in this.actualRootItem.saveParameter.Rules)
                    {
                        if (r.Enabled)
                        {
        ...
        private void createSaveParams(ImageTreeViewItemCor root, Rule r)
        {
            if (!ExistItem(r.Name))
            {
        ...
        private bool ExistsRule(Rule r)
        {
        ...
        private ImageTreeViewItemCor searchRuleParent(Rule rule)
        {
        ...
        private void updatePreviewInformation()
        {
            ...
            foreach (Rule r in actualRootItem.saveParameter.Rules)
            {
                if (r.Enabled)
                {
